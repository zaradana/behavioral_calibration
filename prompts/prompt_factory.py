...
class PromptFactory:
    @staticmethod
    def get_prompt(benchmark_config: BenchmarkConfig, processed_instance: ProcessedInstance, target_threshold: float) -> str:
        ...
        if "swe" in benchmark_config.dataset_name.lower():
            return get_swe_prompt(processed_instance.prompt_data, behavioral_calibration_prompt)
        elif "gpqa" in benchmark_config.dataset_name.lower():
            return get_gpqa_prompt(processed_instance.prompt_data, behavioral_calibration_prompt)
        else:
            # For other datasets, default to proxy style prompt
            return get_proxy_prompt(processed_instance.prompt_data, behavioral_calibration_prompt)
