...
def _looks_like_unified_diff(text: str) -> bool:
    ...
    starts_properly = (
        t.startswith("---") or t.startswith("diff --git") or t.startswith("Index:")
    )

    return (
        has_headers
        and has_hunk
        and not has_fenced_blocks
        and starts_properly
        and has_clean_structure
    )
...
def prepare_swebench_predictions(
    predictions: List[ItemEval], model_name: str
) -> List[Dict[str, Any]]:
    """
...
def run_swebench_with_docker(
    ...
    if platform.system() == "Darwin" and not os.environ.get("DOCKER_HOST"):
        ...
        if os.path.exists(docker_sock_path):
            ...
            logger.info(
                f"Set DOCKER_HOST to {os.environ['DOCKER_HOST']} for macOS Docker Desktop"
            )
    ...
    try:
        ...
        logger.info(
            f"SWE-bench evaluation completed. Results in evaluation_results/{run_id}/"
        )
